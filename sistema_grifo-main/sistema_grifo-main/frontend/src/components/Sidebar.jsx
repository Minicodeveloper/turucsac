  ...
  return (
    <aside className="hidden md:flex flex-col pt-20 pb-8 h-screen w-64 glass fixed left-0 top-0 z-40 shadow-[16px_0_32px_rgba(11,29,45,0.04)]">
      <div className="px-8 mb-6 flex items-center gap-4">
        <div className="w-11 h-11 rounded-xl shadow-sm border border-[var(--outline-variant)]/20 flex items-center justify-center overflow-hidden flex-shrink-0">
          <img src="/sistema_grifo-main/frontend/dist/logos/brand_grifo.svg" alt="Logo Grifo" className="w-full h-full object-contain p-1.5" />
        </div>
        <div>
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--on-surface-variant)] mb-1">Operaciones</h2>
          <p className="text-[11px] text-[var(--outline-variant)] font-bold tracking-tight text-primary-container">Gestión Integral</p>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto space-y-1 mb-4 custom-scrollbar">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <div className="absolute left-0 w-[4px] h-6 bg-[var(--primary-container)] rounded-r-full shadow-[0_0_8px_rgba(0,174,239,0.3)]"></div>
              )}
